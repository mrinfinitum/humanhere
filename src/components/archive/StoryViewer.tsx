...
export function StoryViewer({ entry, previous, next }: { entry: HumanEntry; previous?: HumanEntry; next?: HumanEntry }) {
  ...
  return <main className="story-viewer">
    ...
    <article>
      ...
      {entry.blocks?.map(block => block.type === "text" ? <section className="story-copy" key={block.id}>{block.heading && <h2>{block.heading}</h2>}{block.body.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</section> : block.type === "quote" ? (block.quote.trim() === entry.quote?.trim() ? null : <blockquote className="story-quote" key={block.id}>“{block.quote}”</blockquote>) : block.type === "note" ? <aside className="story-note" key={block.id}><p>{block.text}</p>{block.attribution && <small>{block.attribution}</small>}</aside> : null)}
