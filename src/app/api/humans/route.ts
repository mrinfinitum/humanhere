...
export async function GET(request: NextRequest) {
  ...
  const batch = await humanArchiveRepository.listPublished({ cursor, limit: Number.isFinite(limitValue) ? limitValue : 12, types: requestedTypes.length ? requestedTypes : undefined });
