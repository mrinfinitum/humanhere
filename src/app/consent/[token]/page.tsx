...
async function CreatorConsentContent({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (token.length < 40 || token.length > 100) notFound();
  const admin = createSupabaseAdminClient();
  const { data } = await admin.from("social_creator_consent_tokens").select("id,expires_at,used_at").eq("token_hash", hashCreatorConsentToken(token)).is("used_at", null).filter("expires_at", "gt", "now()").maybeSingle() as unknown as { data: { id: string; expires_at: string; used_at: string | null } | null };
  if (!data) notFound();
